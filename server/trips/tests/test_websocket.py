...
@database_sync_to_async
def create_trip(
        pick_up_address='124 Main',
        drop_off_address='456 Road',
        status='REQUESTED',
        rider=None,
        driver=None
):
...
class TestWebSocket:
    ...
    async def test_request_trip(self, settings):
        ...
        user, access = await create_user(
            'test@example.com', 'pAssw0rd', 'rider'
        )
        communicator = WebsocketCommunicator(
            application=application,
            path=f'/taxi/?token={access}'
        ...
        assert response_data['pick_up_address'] == '123 Main st'
        assert response_data['drop_off_address'] == '456 Main'
    ...
    async def test_driver_alerted_on_request(self, settings):
        ...
        await communicator.send_json_to({
         'type': 'create.trip',
         'data': {
             'pick_up_address': '123 Main',
             'drop_off_address': '456 Piney',
             'rider': user.id
         }
        })
    ...
    async def test_create_trip_group(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        user, access = await create_user(
            'test@example.com', 'pAsswo0d', 'rider'
        )
        communicator = WebsocketCommunicator(
            application=application,
            path=f'/taxi/?token={access}'
    ...
    async def test_join_trip_group_on_connect(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        user, access = await create_user(
            'test@example.com', 'pAssw0d', 'rider'
        )
    ...
    async def test_driver_can_update_trip(self, settings):
        ...
        rider, _ = await create_user(
            'testrider@example', 'pAsswor0d', 'rider'
        )
        ...
        driver, access = await create_user(
            'testdriver@example.com', 'pAsswor0', 'driver'
        )
        communicator = WebsocketCommunicator(
            application=application,
            path=f'/taxi/?token={access}'
    ...
    async def test_driver_join_group_on_connect(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        user, access = await create_user(
            'test@example.com', 'pAsswo0d', 'driver'
        )
